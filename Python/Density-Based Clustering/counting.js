...
function draw() {
  ...
  for (let i = 0; i < width; i += SQUARE_SIDE) {
    for (let j = 0; j < height; j += SQUARE_SIDE) {
      ...
      counts[i / SQUARE_SIDE].push(count);
    }
  }
  ...
  for (let i = 0; i < width; i += SQUARE_SIDE) {
    for (let j = 0; j < height; j += SQUARE_SIDE) {
      square_color = map(counts[i / SQUARE_SIDE][j / SQUARE_SIDE], 0, maximum(counts), 0, 255);
      fill(0, square_color, 0);
      rect(i, j, SQUARE_SIDE, SQUARE_SIDE);
      // fill('white');
      // textSize(SQUARE_SIDE / 3);
      // text(counts[i / SQUARE_SIDE][j / SQUARE_SIDE], i + SQUARE_SIDE / 2, j + SQUARE_SIDE / 2);
    }
  }
